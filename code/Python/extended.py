    ...
    for folder in source_folders:
        ...
        for source_file in source_file_list:
            if 'stats' in source_file:
                ...
                middle_eyes = source.assign(blick_m_x = pd.to_numeric(source.blick_l_x) / 2 + pd.to_numeric(source.blick_r_x) / 2, blick_m_y = pd.to_numeric(source.blick_l_y) / 2 + pd.to_numeric(source.blick_r_y) / 2)
                result = middle_eyes.assign(delta_l_t = lambda x : np.sqrt(np.power(pd.to_numeric(x.blick_l_x) - pd.to_numeric(x.pix_x),2) + np.power(pd.to_numeric(x.blick_l_y) - pd.to_numeric(x.pix_y),2)), delta_r_t = lambda x : np.sqrt(np.power(pd.to_numeric(x.blick_r_x) - pd.to_numeric(x.pix_x),2) + np.power(pd.to_numeric(x.blick_r_y) - pd.to_numeric(x.pix_y),2)), delta_m_t = lambda x : np.sqrt(np.power(pd.to_numeric(x.blick_m_x) - pd.to_numeric(x.pix_x),2) + np.power(pd.to_numeric(x.blick_m_y) - pd.to_numeric(x.pix_y),2)))

...
Use the command \textit{$\backslash$title} and follow the given structure in "example.tex". All
letters of the title should be capitalized but for the subtitle only initial letters should have this format. If no subtitle is required, please remove the corresponding \textit{$\backslash$subtitle} command. In the subtitle, words like "is", "or", "then", etc. should not be capitalized unless they are the first word of the subtitle. No formulas or special characters of any form or language are allowed in the title or subtitle.
